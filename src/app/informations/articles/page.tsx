...
export default function Articles() {
  ...
  const filteredPolicy = articlesData.filter(
    (item) =>
      item.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.exceprt.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="flex flex-col min-h-screen bg-main">
      ...
      <div className="flex flex-col min-h-screen bg-white px-24">
        <div className="flex justify-center">
          <input
            type="text"
            placeholder="Cari berdasarkan judul artikel atau topik artikel"
            value={searchTerm}
            onChange={(e) => {
        ...
        {paginatedPolicy.map((policy, index) => (
          <InfoCard
            key={index}
            title={policy.title}
            description={policy.exceprt}
            yearApplied={formatDateToMonthYear(policy.date)}
            link={`/informations/articles/${policy.slug}`}
            infoCardType="article"
          />
        ))}
